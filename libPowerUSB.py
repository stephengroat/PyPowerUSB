...
bus[0].devices[0].idProduct #should be 63L
handle=bus[0].devices[0].open()
try:
...
def send_msg(msg):
    ...
    handle.bulkWrite(1,msg+padding,200)

def _read_msg(msg,reply_len):
    send_msg(msg)
    return handle.bulkRead(1,64,200)[0:reply_len] #significant reply
